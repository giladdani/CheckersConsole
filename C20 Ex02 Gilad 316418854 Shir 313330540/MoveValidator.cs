...
namespace C20_Ex02_Gilad_316418854_Shir_313330540
{
    ...
    {
        // check if the player can capture========================
        public static bool IsPlayerHasCapture(Player i_Player, Board i_Board)
        {
            bool capturePossibale=false;
            
            for(int i=0;i<i_Player.Pieces.Count;i++)
            {
                if(i_Player.Pieces[i].IsKing)
                {
                    capturePossibale = IsKingCapturePossible(i_Player, i_Board, i_Player.Pieces[i]);
                }
                else 
                {
                    capturePossibale = IsCapturePossiblePerPiece(i_Player, i_Board, i_Player.Pieces[i]);
                }
            }
            
            return capturePossibale;
        }
